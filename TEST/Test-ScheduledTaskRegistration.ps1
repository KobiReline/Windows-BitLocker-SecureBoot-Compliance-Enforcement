...
foreach ($source in @(
    ...
    @{ File = 'SecurityFeatureMonitor-Backend.ps1'; Names = @('Set-BackendScheduledTask', 'Get-NextIntervalMinutes', 'Test-TaskRepairRequired', 'Get-TaskInstancePolicy') }
)) {
    $tokens = $null
    $errors = $null
...
function Assert-RegisteredTask {
    ...
    if ((Get-TaskInstancePolicy -Task $task) -ne 'StopExisting') { throw "Wrong instance policy: $Name" }
