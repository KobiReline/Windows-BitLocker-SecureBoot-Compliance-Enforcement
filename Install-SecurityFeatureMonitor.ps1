...
function Install-ScriptFiles {
    ...
    Copy-Item -LiteralPath (Join-Path $SourceDirectory 'Invoke-SecurityFeatureMonitor.ps1') -Destination $InstallDirectory -Force
...
function Register-UserInterfaceTask {
    ...
    $action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy AllSigned -File `"$uiPath`""
...
Write-Output 'Security Feature Monitor UI installed successfully.'
